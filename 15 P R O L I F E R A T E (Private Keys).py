...
if __name__ == "__main__":
    ...
    if len(key_input) in [51, 52]:
        main_priv_key = wif_to_private_key(key_input)
    elif len(key_input) == 64:
        main_priv_key = bytes.fromhex(key_input)
    else:
        main_priv_key = mnemonic_to_private_key(key_input)
    ...
    for i in range(batch_count):
        ...
        new_priv_bytes = os.urandom(32)
        
        payload = b'\xb0' + new_priv_bytes + b'\x01'
        checksum = hashlib.sha256(hashlib.sha256(payload).digest()).digest()[:4]
        new_wif = base58.b58encode(payload + checksum).decode('utf-8')
