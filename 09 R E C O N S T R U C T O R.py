...
def fetch_and_reconstruct(address, start_block, end_block):
        ...
        if start_block <= height <= end_block:
            ...
            if flo_data and " | " in flo_data:
                try:
                    ...
                    chunks[int(header)] = payload
                    total_found += 1
                    print(f"Found {header} in Block {height}")
    ...
    if data_keys:
        ...
        if missing_chunks:
            print(f"[WARNING] Gaps detected! Missing: {', '.join(f'{m:04d}' for m in sorted(missing_chunks))}")
    ...
    try:
        ...
        print(f"[ERROR] Failed to write file to disk: {e}")
        print("-" * 44)
